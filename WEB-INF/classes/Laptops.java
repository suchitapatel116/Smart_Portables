...
public class Laptops extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		//List<Product> laptops_list = productsDataStore.getLaptopList();
		List<Product> laptops_list = ProductsHashMap.getLaptopList();

		HttpSession session = request.getSession();

		Product product;
		if(laptops_list == null)
		{
			contentData += "null data</div>";
		}
		else if(laptops_list != null)
		{
			contentData += "<div id='holder'>"
							+ "<h3 id='product_header'>Laptops</h3>"
			...
			{
				product = (Product)laptops_list.get(i);
